...
export default function App() {
  ...
  function addNewCharger() {
    try {
      const newCharger = mockApi.addNewCharger();
      dispatch(addCharger(newCharger));
    } catch (err) {
      if (err instanceof Error) setError(err.message);
  ...
  return (
    <>
      <div className="w-full max-w-4xl mx-auto my-2 h-lvh p-6 space-y-4 lg:bg-gray-100">
        <h1 className="sm:text-lg md:text-xl lg:text-2xl font-bold text-center">
        ...
        <div className="flex gap-10 w-full justify-between">
          ...
          <p className="text-center sm:text-lg md:text-xl lg:text-2xl text-red-600 font-bold">
            {error}
          </p>
        </div>
        <ChargersList />
